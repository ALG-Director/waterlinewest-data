...
This script intentionally updates only one indicator. The rest of the
...
def load_json_from_url(url: str, attempts: int = 3, timeout: int = 45) -> dict:
    """Fetch JSON with a few retries so one slow response doesn't fail the run."""
    ...
    for attempt in range(1, attempts + 1):
        ...
        if attempt < attempts:
            time.sleep(5 * attempt)  # 5s, then 10s, before giving up
    raise last_error
